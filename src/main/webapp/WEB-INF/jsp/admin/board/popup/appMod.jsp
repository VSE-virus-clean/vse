	    ...
	    <form name="mainForm" method="post" enctype="multipart/form-data" onsubmit="var rtn = formSubmitObj.submit(this); if(!rtn){ submitUtil.enable(); } return rtn;">
	        ...
	        <table class="r_search_table">
	            ...
	            <tr>    
	                ...
	                <td colspan="3">
	                    <input type="text" id="expsRgstDay" name="expsRgstDay" class="datepicker date_input" readonly="readonly" autocomplete="off" value="${result.info.expsRgstDay}" />
	            ...
	            <tr>    
	                ...
	                <td colspan="3">
	                    <input type="text" id="expsFnhDay" name="expsFnhDay" class="datepicker date_input" readonly="readonly" autocomplete="off" value="${result.info.expsFnhDay}" />
	                    <select name="expsFnhTime" style="width:50px; margin-left:30px;" >
	                    	<c:forEach var="time" begin="0" end="23" varStatus="i">
	                    		<option value="${time < 10 ? '0' : '' }${time}" >${time}</option>
	                    	</c:forEach>
	                    </select> 시
	            ...
	            <tr>    
	                <th>URL</th>
	                <td colspan="3">
	                    <input type="text" id="rltdLk" name="rltdLk" maxlength="1000" style="width:80%;" title="연결 URL"  value="${result.info.rltdLk}"/>
	                </td>   
	            </tr>
	            <tr>    
	                <th class="necessary">내용</th>
	                <td colspan="3">
	                    <textarea name="blcSbc1" id="blcSbc1" style="width:100%;height:500px;" title="내용">${result.info.blcSbc1}</textarea>    
	                </td>   
	            </tr>
	            <tr>    
	                <th class="necessary">팝업이미지</th>
	                <td colspan="3" class="con">
	                	<div class="filebox">
		                	<p>${function:printImageFile("THUMB", result.file.list)}</p>
		                    ${function:printAttachFileModList2("Y", 1, "THUMB", "팝업이미지", result.file.list, "Y")}
	                    </div>
	                    <ul class="warn">
